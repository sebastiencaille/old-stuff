...
public class MLDonkeyGui {
    ...
    protected void refreshSharedFiles() {
        ...
        final var current = Set.of(Objects.requireNonNull(this.incoming.listFiles(), "Files cannot be listed"));
    ...
    private static class StreamReader extends Thread {
        ...
        public void run() {
            try (var in = this.stream.get();
                ...
                if (show) {
                    try (var bufferedReader = new BufferedReader(new InputStreamReader(in))) {
                        Logs.of(MLDonkeyGui.class).info(() -> bufferedReader.lines().collect(Collectors.joining("\n")));
                    }
                } else {
                    in.transferTo(out);
                }
